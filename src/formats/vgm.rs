...
pub fn unsupported_chip_name(cmd: u8) -> &'static str {
    match cmd {
        0x52 | 0x53 => "YM2612 (OPN2 FM)",
        0x54 => "YM2151 (OPM FM)",
        0x55 => "YM2203 (OPN FM)",
        0x56 | 0x57 => "YM2608 (OPNA FM)",
        0x58 | 0x59 => "YM2610 (OPNB FM)",
        ...
        0xBF => "GA20",
        0xC0 => "Sega PCM",
        0xC1 => "RF5C68 (memory write)",
        0xC2 => "RF5C164 (memory write)",
...
pub struct VgmFile {
    ...
    /// (subtly different DAC curve) rather than a plain AY-3-8910 — see chips::ay8910::Ayumi::new.
...
pub fn parse(bytes: &[u8]) -> Result<VgmFile, String> {
    ...
    while pos < data.len() {
        ...
        match cmd {
            ...
            // Other chips' plain "register, value" writes (YM2612, YM2151, YM2203, YM2608,
            // YM2610, Y8950, YMZ280B, YMF262, a second AY8910, RF5C68, RF5C164, PWM,
            // GameBoy DMG, NES APU, MultiPCM, uPD7759, OKIM6258, OKIM6295, HuC6280, K053260,
            // Pokey, WonderSwan, SAA1099, ES5506, GA20, ...) — a uniform 3-byte encoding (cmd,
            // register, value) across the whole VGM spec, safe to skip even though we don't
            // emulate these chips yet. YM3812/YM3526 (0x5A/0x5B) are carved out above, not here.
            0x52..=0x59 | 0x5C..=0x5F | 0xA1..=0xAF | 0xB0..=0xBF => {
                *unsupported_commands.entry(cmd).or_insert(0) += 1;
                pos += 3;
            }
            // Chips addressed by a wider offset/register space (Sega PCM, RF5C68/164,
            // MultiPCM bank select, QSound, SCSP, WonderSwan, VSU, X1-010: cmd + 16-bit
            // address + value; YMF278B, YMF271, K054539, C140, ES5503, ES5506: cmd + port +
            // register + value) — both a uniform 4-byte encoding per the VGM spec, same
            // family K051649/SCC (0xD2, handled above) belongs to.
            0xC0..=0xC8 | 0xD0 | 0xD1 | 0xD3..=0xD6 => {
                *unsupported_commands.entry(cmd).or_insert(0) += 1;
                pos += 4;
            }
            0x61 => {
