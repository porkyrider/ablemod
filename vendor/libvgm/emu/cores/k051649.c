...
static void k051649_update(void *param, UINT32 samples, DEV_SMPL **outputs)
{
	...
	for (j = 0; j < 5; j++)
	{
		...
		{
			// ablemod patch: this line's original "/ 2.0f" made every channel play exactly
			// one octave sharp. With info->rate == info->mclock/16 (see device_start_k051649
			// below), the original formula reduces to freq_hz = rate/(freq+1) =
			// mclock/(16*(freq+1)) -- double the well-established, independently verified
			// real-hardware formula freq_hz = mclock/(32*(freq+1)) (matches MAME's own current
			// k051649.cpp, openMSX, and a direct autocorrelation measurement against real VGM
			// audio -- see this project's README.md and formats::vgm's own comment on the
			// K051649 clock). Removing "/ 2.0f" restores that formula exactly; caught by
			// chips_tests.rs's test_scc_tone_frequency_matches_the_chips_own_formula and
			// test_scc_solo_silences_every_other_channel both measuring 2.00x the expected
			// frequency before this patch.
			UINT32 step = (UINT32)(((INT64)info->mclock * (1 << FREQ_BITS)) / (float)((voice[j].frequency + 1) * info->rate) + 0.5f);
